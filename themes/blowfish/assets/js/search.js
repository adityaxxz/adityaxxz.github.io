...
function executeQuery(term) {
  ...
  // Always show resume link if term includes "resume"
  const showResumeLink = term.toLowerCase().includes("resume");
  
  if (results.length > 0 || showResumeLink) {
    ...
    // Add resume link if searching for "resume"
    if (showResumeLink) {
      resultsHTML =
        resultsHTML +
        `<li class="mb-2">
          <a class="flex items-center px-3 py-2 rounded-md appearance-none bg-neutral-100 dark:bg-neutral-700 focus:bg-primary-100 hover:bg-primary-100 dark:hover:bg-primary-900 dark:focus:bg-primary-900 focus:outline-dotted focus:outline-transparent focus:outline-2" 
          href="https://drive.google.com/file/d/1BtqmPf-sFE3H92xlH6je29VA8_NMoP6_/view" target="_blank" rel="noopener" tabindex="0">
            <div class="grow">
              <div class="-mb-1 text-lg font-bold">
                Resume
              </div>
              <div class="text-sm text-neutral-500 dark:text-neutral-400">Document<span class="px-2 text-primary-500"></span></span></div>
              <div class="text-sm italic">View or download my resume</div>
            </div>
            <div class="ml-2 ltr:block rtl:hidden text-neutral-500">&rarr;</div>
            <div class="mr-2 ltr:hidden rtl:block text-neutral-500">&larr;</div>
          </a>
